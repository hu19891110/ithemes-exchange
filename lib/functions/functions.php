		...
		if ( empty( $_GET['remove-gateway'] ) || 'yes' !== $_GET['remove-gateway'] ) {
			switch( $add_on ) {
				...
				case 'zero-sum-checkout':
					$title = __( 'Payment Gateway Warning', 'it-l10n-ithemes-exchange' );
					$yes = '<a href="' . esc_url( add_query_arg( 'remove-gateway', 'yes' ) ) . '">' . __( 'Yes', 'it-l10n-ithemes-exchange' ) . '</a>';
					$no  = '<a href="javascript:history.back()">' . __( 'No', 'it-l10n-ithemes-exchange' ) . '</a>';
					$message = '<p>' . sprintf( __( 'Deactivating a payment gateway can cause customers to lose access to any membership products they have purchased using this payment gateway. Are you sure you want to proceed? %s | %s', 'it-l10n-ithemes-exchange' ), $yes, $no ) . '</p>';
					$args = array(
						'response'  => 200,
						'back_link' => false,
					);
